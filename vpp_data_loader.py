...
def read_csv_with_fallback(filepath, **kwargs):
    """
    Read a CSV trying multiple encodings AND multiple delimiters.

    Strategy:
    1. Try each (separator, encoding) combo with the C engine
    2. Prefer results with more than 1 column (multi-column structure)
    3. If we get a metadata-header file (line 1 has fewer fields than data lines),
       retry skipping initial rows
    4. Last resort: Python engine with sep=None for auto-detection

    This is NOT cached because caching is done by the callers
    (load_price_data, load_all_data). Caching here would make debugging harder
    and cause stale reads when the CSV changes on disk.
    """
    encodings = ['utf-8-sig', 'utf-8', 'cp1252', 'latin-1']
    separators = [',', ';', '\t', '|']
    last_error = None
    single_column_fallback = None

    # Attempt 1: standard read with every (sep, encoding) combo
    for sep in separators:
        for enc in encodings:
            try:
                ...
                if df.shape[1] > 1:
                    return df
                # Keep a single-column result as a last-resort backup
                if single_column_fallback is None:
                    single_column_fallback = df
    ...
    # Attempt 2: try skipping leading metadata rows (common in exports)
    # Probe rows 1–5 as candidate header locations
    for skip in range(1, 6):
        for sep in separators:
            for enc in encodings:
                try:
                    df = pd.read_csv(filepath, encoding=enc, sep=sep, skiprows=skip, **kwargs)
                    if df.shape[1] > 1 and len(df) > 10:
                        return df
                except Exception as e:
                    last_error = e
                    continue

    # Attempt 3: Python engine with auto-sniff
    for enc in encodings:
        try:
    ...
    # If we got any single-column result earlier, return it
    if single_column_fallback is not None:
        return single_column_fallback

    raise last_error if last_error else ValueError(f'Could not parse {filepath}')
